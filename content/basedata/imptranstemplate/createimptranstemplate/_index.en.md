...
- **_cac_**: **Currency of the bank account**, an [ISO 4217](https://de.wikipedia.org/wiki/ISO\_4217) value is expected. This is used to determine the bank account.
...
- **_transType_**: This is an assignment of the text to a **transaction type**. The configuration consists of the transaction type separated by "|" one or more texts. For example "transType=DIVIDEND|Interest,Dividend" whereby this also corresponds to the two line configuration "transType=DIVIDEND|Interest" and "transType=DIVIDEND|Dividend". The following transactions are supported in CSV and PDF templates:
  ...
  - **FINANCE_COST**: Is the [finance cost](../../../transaction/security/marginbased/) of an open margin position (Forex and CFD). The **units** field contains the number of days and the **quotation** field the finance cost per day. When the transaction is created, GT automatically links the finance cost to the open position of the instrument in the corresponding securities account. This only succeeds if exactly one open position exists at the transaction time, otherwise the import position receives an error message and the transaction must be entered manually. Opening and closing margin positions is not supported by the import.
  - **transType only valid for CSV**: The following **transaction types** can also be used in an import template for **CSV**:
    - **DEPOSIT**: Is a deposit into an account.
    - **FEE**: Costs for account management or custody account fees.
    - **INTEREST_CASHACCOUNT**: Is the interest for an account.
    - **WITHDRAWAL**: Is a withdrawal from an account.
...
- **calcRounding**: When the **total amount** is calculated from the imported values, tiny rounding differences from the **total amount** shown in the document can occur. This configuration defines up to which amount such a deviation is accepted automatically. The **import position** then becomes **transaction-capable** without manual intervention, the **total amount** shown in the document is credited to the bank account, and the deviation is recorded as a **rounding difference** on the transaction. The first value is the default; in addition, deviating values can be specified per currency. For example, "calcRounding=0.01" accepts a deviation of one Rappen or cent, and "calcRounding=0.01,JPY=1,CHF=0.05" uses a whole unit for the Japanese yen and five Rappen for the Swiss franc (Swiss rounding). This configuration applies to **CSV** and **PDF** templates.
