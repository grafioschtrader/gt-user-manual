...
- **_cac_**: **Währung des Bankkontos**, es wird ein [ISO 4217](https://de.wikipedia.org/wiki/ISO_4217) Wert erwartet. Damit wird das Bankkonto bestimmt.
...
+ **_transType_**: Diese ist eine Zuweisung des Textes zur einer **Transaktionsart**. Die Konfiguration besteht aus der Transaktionsart getrennt durch "|" einen oder mehrere Texte. Beispielsweise "transType=DIVIDEND|Zins,Dividende" wobei auch dies der zwei Zeilen-Konfiguration "transType=DIVIDEND|Zins" und "transType=DIVIDEND|Dividende" entspricht. Folgende Transaktionen werden in CSV und PDF Vorlagen unterstützt:
   ...
   - **FINANCE_COST**: Sind die [Finanzierungskosten](../../../transaction/security/marginbased/) einer offenen Margin-Position (Forex und CFD). Das Feld **units** enthält die Anzahl Tage und das Feld **quotation** die Finanzierungskosten pro Tag. Beim Erstellen der Transaktion verknüpft GT die Finanzierungskosten automatisch mit der offenen Position des Instrumentes im entsprechenden Wertpapierdepot. Dies gelingt nur, wenn zum Transaktionszeitpunkt genau eine offene Position besteht, andernfalls erhält die Importposition eine Fehlermeldung und die Transaktion muss manuell erfasst werden. Das Öffnen und Schliessen von Margin-Positionen wird durch den Import nicht unterstützt.
   ...
   - **transType nur für CSV gültig**: Folgende **Transaktionsarten** können zusätzlich in einer Importvorlage für **CSV** angewendet werden: 
     - **DEPOSIT**: Ist eine Einzahlung auf ein Konto.
     - **FEE**: Kosten für die Kontoführung oder Depotgebühren.
     - **INTEREST_CASHACCOUNT**: Ist der Zins für ein Konto.
     - **WITHDRAWAL**: Ist eine Auszahlung von einem Konto.
...
+ **calcRounding**: Beim Errechnen der **Gesamtsumme** aus den importierten Werten können winzige Rundungsdifferenzen gegenüber der im Dokument ausgewiesenen **Gesamtsumme** entstehen. Mit dieser Konfiguration wird festgelegt, bis zu welcher Höhe eine solche Abweichung automatisch akzeptiert wird. Die **Importposition** wird dann ohne manuelles Eingreifen **Transaktionsfähig**, die im Dokument ausgewiesene **Gesamtsumme** wird dem Bankkonto gutgeschrieben und die Abweichung als **Rundungsdifferenz** auf der Transaktion festgehalten. Der erste Wert gilt als Standard, zusätzlich können pro Währung abweichende Werte angegeben werden. Beispielsweise akzeptiert "calcRounding=0.01" eine Abweichung von einem Rappen oder Cent, und "calcRounding=0.01,JPY=1,CHF=0.05" verwendet für den japanischen Yen eine ganze Einheit und für den Schweizer Franken fünf Rappen (Rappenrundung). Diese Konfiguration gilt für **CSV**- und **PDF**-Vorlagen.
