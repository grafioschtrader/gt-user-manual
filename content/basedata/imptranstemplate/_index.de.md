...
{{% notice style="info" title="Vorlagengruppe «Grafioschtrader»" %}}
Nebst den Vorlagengruppen für Handelsplattformen gibt es eine **einzigartige**, mit GT mitgelieferte Vorlagengruppe «**Grafioschtrader**». Sie enthält die Vorlagen, um die eigenen Exporte von GT, den CSV-Export der Transaktionen und die Transaktionsbelege, wieder einzulesen. Diese Vorlagengruppe sollte weder bearbeitet noch ein zweites Mal angelegt werden, siehe [Grafioschtrader-Import](../../tenantportfolio/securityaccounts/transactionimport/grafioschtrader/). Damit die Klienten sie benutzen können, muss ein **Administrator** sie einmalig dafür bestimmen: Vorlagengruppe auswählen und im Kontextmenü **Als Grafioschtrader-Vorlagengruppe verwenden** wählen. Diese Wahl gilt für die ganze Installation; der einzelne Klient entscheidet danach nur noch selbst, ob er die Vorlagen benutzen möchte, siehe [Klient](../../tenantportfolio/client/).
{{% /notice %}}
