...
- **GBX divider**: Whether prices should automatically be divided by 100 — required for exchanges that quote in the minor unit of the currency: the London Stock Exchange quotes in pence (GBX or GBp) instead of pounds, the Johannesburg Stock Exchange in cents (ZAc) instead of rand. The division is only applied to securities of these exchanges with the corresponding currency (GBP or ZAR); indices are exempt.
