...
archetype: "default"
---
This evaluation contains the **annual return** and several **statistical metrics** for a **single instrument**. It is shown when a row in the [correlation matrix]({{< relref "/watchlistinstrument/correlation" >}}) is expanded. The same evaluation is also available in the expanded table row of the [quote data feed view]({{% relref "/watchlistinstrument/watchlist/pricefeed" %}}), where you open it through the section "Return and statistical data".

- **Period**: The **return** is computed over the entire span of available end-of-day prices. The period for the **statistical metrics** can be narrowed via the date fields of the correlation matrix. The quote data feed view does not offer these date fields, so the entire span is always evaluated there.
...
The date fields of the correlation matrix affect every statistical metric — including the Sharpe ratio described in the next section. The return calculation above continues to use the entire span of available closing prices regardless. When you open the evaluation from the quote data feed view the restriction disappears entirely, because that view offers no date fields.
{{% /notice %}}
