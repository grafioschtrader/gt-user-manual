...
- **GTNet as fallback**: If the GTNet functionality is enabled and the "Receive historical price data" or "Receive intraday price" option is set on the instrument, price data can also be obtained via GTNet when the configured connector is not working (or has never worked). Configuring a working connector as the primary data source is still recommended. More on this in [Retry Counter and GTNet Fallback](../../admindata/gtnet/quoteretry/).
