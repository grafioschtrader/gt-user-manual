...
Once a retry counter reaches the connector limit, GTNet can step in as a fallback if the "Receive historical price data" or "Receive intraday price" option is enabled for the instrument. GT then attempts additional updates via GTNet without resetting the retry counter to 0. The connector failure therefore remains visible in this view and in the [connector monitoring](../../../admindata/taskdatachangemonitor/), while you continue to receive price data. A detailed description of the bands is given under [Retry Counter and GTNet Fallback](../../../admindata/gtnet/quoteretry/), and the configuration under [Global Settings GTNet](../../../admindata/gtnet/globalsettings/).
...
## Expanding table row
The expanded table row offers two collapsible sections, both of which are **closed** when you expand a row. This keeps the row compact and loads only the data you actually want to see. Clicking the title opens or closes the corresponding section.

### Instrument and price feed data
This section shows almost all the properties of the instrument. The display is grouped for a better overview, for example into **Base data** and **Quotation data** as well as the groups **Historical price data connector**, **Intraday connector**, **Dividend Connector** and **Split connector**. For a derived instrument the group **Derived instrument** is added. Depending on the instrument, both the properties displayed and their grouping may vary.

### Return and statistical data
This section contains the same evaluation that the correlation matrix offers when a row is expanded. Its full description is given under [Return and statistical data]({{% relref "/watchlistinstrument/correlation/returnstatdata" %}}). One difference is worth noting: this watchlist has no date fields, so the statistical metrics here always cover the entire span of available end-of-day prices.

The evaluation is calculated only when you open the section for the first time. After that it is available immediately for as long as you work with the watchlists, because neither collapsing a row nor switching the watchlist or the view triggers a new calculation. GT also remembers per instrument which sections you have opened.
