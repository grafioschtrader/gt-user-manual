...
- **Transaction receipts**: Generates PDF receipts for one or more **security transactions** of the selected instrument, see [Transaction receipts](../../transaction/receipt/).
...
**Turn on/off filter row**: This shows or hides the filter row below the column headers, see [Filtering and sorting](filtersort).
**Filter and sorting settings...**: In this dialog you define whether a filter or a sort order applies only to this watchlist or to all watchlists; filters and sort orders which are set can also be removed here, see [Filtering and sorting](filtersort).
