...
A small installation with a few users already runs on a private [Raspberry Pi 4](https://www.raspberrypi.org/products/raspberry-pi-4-model-b/). You can get a TLS certificate free of charge from [Let's Encrypt](https://letsencrypt.org/). Free dynamic DNS is available from [DuckDNS](https://www.duckdns.org/), for example. Unfortunately, the upload speed of many Internet providers is still very low, which can lead to delays when accessing your home server connected to the Internet.
