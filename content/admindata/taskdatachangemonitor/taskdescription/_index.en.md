...
## 22 - Aggregate GTNet exchange logs
This task type aggregates GTNet exchange log entries from shorter to longer time periods in a rolling manner. The aggregation occurs in multiple levels: individual entries are aggregated into daily summaries, daily into weekly, weekly into monthly, and monthly into yearly. Each aggregation level has a configurable threshold that determines when entries are promoted to the next level. The default schedule runs daily at 3:00 AM UTC. This aggregation reduces storage requirements while preserving statistical data about GTNet exchange activity over time.
