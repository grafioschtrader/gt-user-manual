...
As a further protection against misuse, the number of messages a user with the role **Limits** may send per day is capped. The administrator controls this in the [Limit information class]({{% relref "/admindata/entitylimit" %}}) view through the limit **Message** of the limit type **Changes per day** (default 200). For users with privileged roles (Administrator or without limits) no such limit is recorded from the outset, so they are unrestricted; the administrator can create one at any time if needed.
...
| System         | Administrator | **Holdings tables deviate from the transactions**: The daily check of the [background task]({{% relref "/admindata/taskdatachangemonitor/taskdescription/#JOB54" %}}) found position entries that do not match the transactions. It is the only message type that can also be switched off entirely, so that a deviation that is already known is not reported again every day. | GT message            |
