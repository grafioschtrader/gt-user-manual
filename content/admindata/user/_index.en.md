...
When a user with limits reaches the day limit of an information object, a dialog appears automatically. The information object concerned is already preset and cannot be changed. The user enters the desired day limit, a To date and a justification, then submits the request. The request appears to the administrator as a limit change proposal and is indicated in the user table by the symbol in column **L**. The administrator can adjust the requested day limit and To date before approval, and then decides on approval or rejection; in both cases the user is notified via the [messaging system]({{% relref "/admindata" %}}).

There is at most one processing limit per user and information object. An approval either creates a new limit or renews the existing one by updating the day limit and the To date; this also reactivates a limit that has already expired. As long as a request for an information object is still open, the user cannot submit a second request for the same information object and is informed that a request is already pending.
