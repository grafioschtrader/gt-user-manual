...
archetype: "default"
---
Dieser Tabelle können Sie die **globalen Einstellungen** von GT entnehmen. Die Veränderung der Eigenschaften ist dem Administratoren vorbehalten.

{{% notice note %}}
Die **Limiten** — beispielsweise die Anzahl möglicher Portfolios pro Klient oder die Anzahl der Änderungen pro Tag — sind nicht mehr Bestandteil dieser Tabelle. Sie werden in der Ansicht [Limite Informationsklasse]({{% relref "/admindata/entitylimit" %}}) verwaltet. In den globalen Einstellungen verbleiben die Schwellenwerte für die Sperrung eines Benutzers, `g.max.security.breach.count` und `g.max.limit.request.exceeded.count`; diese sind keine Limiten auf Daten, siehe [Benutzerrechte]({{% relref "/intro/userrights" %}}).
{{% /notice %}}
...
- "^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,10}$": Mindestens acht und höchstens 10 Zeichen, mindestens ein Grossbuchstabe, ein Kleinbuchstabe, eine Zahl und ein Sonderzeichen.
...
Damit nicht jeder Unsinn eingegeben werden kann. Es gibt eine Validierung für einige Eigenschaftenwerte.  Diese ist gleichzeitig auch ein Hinweis darauf, welche Eingabewerte akzeptiert werden. Dieselbe Syntax wird auch für die Werte der [Limite Informationsklasse]({{% relref "/admindata/entitylimit" %}}) verwendet, dort gehört die Regel jedoch zur Limite selbst und nicht zur einzelnen Zeile.
...
| pattern:^[A-Z]{2,3}$ | 2-3 Grossbuchstaben                  | "CH", "USD"   | "ch", "EURO"    |
