...
{{% /notice %}}
This view shows statistics of data exchange between GTNet instances. Logging must be enabled via the [global setting](../globalsettings/) "g.gnet.use.log". When logging is globally disabled, the "Exchange Log" navigation entry is greyed out and not clickable.
{{% notice style="info" title="Re-login required" %}}
Changes to the global setting "g.gnet.use.log" are only reflected in the navigation tree after logging in again.
{{% /notice %}}
