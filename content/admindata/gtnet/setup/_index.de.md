...
Bevor eine Kommunikation mit anderen Teilnehmern möglich ist, muss der eigene GT-Server eingetragen werden. Dabei ist wichtig, dass der Server von aussen erreichbar ist. Nach der Erfassung des eigenen Servers wird dessen ID als Referenz gespeichert. Anschliessend können weitere erreichbare Instanzen für die Kommunikation erfasst werden. Bleibt der rote Hinweis auf die notwendige Eintragung des eigenen Servers auch nach dem Speichern stehen, konnte die Applikation ihre eigene Adresse nicht wiedererkennen; wie sich das beheben lässt, ist unter [Globale Einstellungen GTNet](../globalsettings/) beim Parameter «g.gnet.my.entry.id» beschrieben.
...
- **Server Online**: Zeigt den aktuellen Online-Status der Instanz an (Online, Offline, Unbekannt oder Ausser Betrieb).
...
- **Neuverbindung angefragt**: Erscheint als hervorgehobenes Symbol, wenn diese Instanz vergeblich versucht hat, die Verbindung neu aufzubauen. Siehe [Verbindung wiederherstellen](troubleshooting/).

### Bedeutung der Status «Online», «Offline», «Unbekannt» und «Ausser Betrieb»
Die Spalte **Server Online** kennt vier Zustände, die sich grundlegend in ihrer Aussage zur Erreichbarkeit unterscheiden.

**Online** bedeutet, dass eine Ping-Nachricht erfolgreich gesendet und eine gültige GTNet-Antwort empfangen werden konnte. Erst eine vollständige Protokollantwort – nicht bereits eine offene TCP-Verbindung – gilt als Online. Damit wird verhindert, dass ein vorgeschalteter Reverse-Proxy oder ein DuckDNS-Platzhalter die Instanz als erreichbar erscheinen lässt, während die GT-Applikation selbst nicht läuft. Antwortet der Peer auf den Ping mit einem HTTP-Fehler (zum Beispiel 502 oder 503), wird die Instanz deshalb als Offline eingestuft.

**Offline** bedeutet, dass der Server nicht oder nicht korrekt antwortet. Es findet keinerlei Datenkommunikation statt. Dieser Status wird automatisch erkannt, wenn der Ping fehlschlägt oder der Server mit einem HTTP-Fehler antwortet. Er kann zusätzlich durch das Senden einer «Server ist jetzt offline»-Nachricht vor dem Herunterfahren proaktiv signalisiert werden.

**Unbekannt** bedeutet, dass der Status nicht geprüft werden kann, weil der ausgehende Handshake mit dem Peer noch nicht abgeschlossen ist – das Token für ausgehende Anfragen fehlt. In diesem Zustand wurde die Instanz zwar bereits angelegt, beispielsweise durch eine eingehende Erstkontakt-Nachricht, eine verlässliche Ping-Anfrage lässt sich aber erst senden, wenn der Handshake von beiden Seiten bestätigt ist. Solche Einträge bleiben mit dem Status «Unbekannt» sichtbar, damit kein veralteter «Online»-Zustand aus einer frühen Handshake-Phase stehen bleibt.

**Ausser Betrieb** bedeutet, dass die Instanz die Einstellung ihres Betriebs angekündigt hat und das angekündigte Datum erreicht ist. Sie wird nicht mehr kontaktiert, und der Zustand wird weder durch eine Statusprüfung noch durch eine eingehende Nachricht dieser Instanz aufgehoben. Wie es dazu kommt und wie sich der Eintrag anschliessend entfernen lässt, ist unter [Wartung und Betriebseinstellung](availability/) beschrieben.

### Unterschied zwischen «Nutzung eingeschränkt» und «Offline»
Der Zustand **Nutzung eingeschränkt** ist unabhängig vom Online-/Offline-Status zu sehen und betrifft nur Server, deren Verbindung tatsächlich nutzbar ist.

**Nutzung eingeschränkt** bedeutet, dass der Server erreichbar ist, aber aufgrund hoher Auslastung keine Datenanfragen bearbeiten kann. In diesem Zustand werden keine Anfragen für Intraday-Kurse oder historische Kurse an diesen Server gesendet. Statusmeldungen wie «Server befindet sich während des Zeitraums im Wartungsmodus», «Der Serverbetrieb wird ab diesem Datum eingestellt» oder «Einstellungen aktualisiert» werden jedoch weiterhin übertragen. Der Server kann eingehende Nachrichten empfangen und darauf antworten. Diese Einstellung kann über die Benutzeroberfläche für den eigenen Server aktiviert werden, um bei Kapazitätsengpässen die Last zu reduzieren.

### Status einer Instanz sofort prüfen
Der Online-Status wird durch die Hintergrundaufgabe **«Online-Status verfolgen»** (Aufgaben-ID 20) rund 30 Sekunden nach dem Serverstart einmalig aktualisiert. Wer nach einem frisch abgeschlossenen Handshake oder bei einem vermuteten Ausfall eines Peers nicht auf den nächsten Start warten möchte, kann im Kontextmenü der Server-Übersicht den Menüpunkt **«Status jetzt prüfen»** verwenden. Dieser sendet sofort einen Ping an die ausgewählte Instanz und aktualisiert die entsprechende Zeile in der Tabelle mit dem Ergebnis. Der Menüpunkt ist nur für Administratoren aktiv und für den eigenen Server-Eintrag deaktiviert.
{{% notice info %}}
Peers ohne abgeschlossenen ausgehenden Handshake werden bei einer sofortigen Prüfung auf «Unbekannt» gesetzt, da ohne gültiges Token für ausgehende Anfragen keine authentifizierte Ping-Nachricht möglich ist.
{{% /notice %}}

### Einen Gegenpart wieder zulassen
Verliert ein Gegenpart seine Zugangsdaten – etwa weil seine Datenbank neu aufgebaut oder eine ältere Sicherung eingespielt wurde –, so wird seine erneute **«Erste Kontaktaufnahme»** hier abgewiesen, solange ein Handshake mit ihm verzeichnet ist. Er kann sich auch nicht auf anderem Weg melden, denn eine Admin-Nachricht setzt genau die Verbindung voraus, die ihm verweigert wird. Damit dieser Fall nicht unbemerkt bleibt, erscheint auf seiner Zeile das Symbol in der Spalte **Neuverbindung angefragt**.

Mit dem Kontextmenüpunkt **«Neuen Handshake erlauben»** oder einem Klick auf dieses Symbol werden die mit diesem Gegenpart geteilten Zugangsdaten verworfen, sodass seine nächste Kontaktaufnahme wieder angenommen wird. Nachrichtenhistorie, Austauscheinstellungen und erteilte Berechtigungen bleiben dabei erhalten. Der ganze Ablauf mit beiden Seiten ist unter [Verbindung wiederherstellen](troubleshooting/) beschrieben.
...
- **[Austausch letzter Preis](../exchange/lastprice/)**: Aktuelle Tageskurse für Wertpapiere und Währungspaare. Wird über die Watchlist ausgelöst.
- **[Austausch historische Kurse](../exchange/historicalprice/)**: Tägliche Schlusskurse für die Vergangenheit. Ermöglicht das Auffüllen von Datenlücken.
...
- **Tägliches Abfragelimit**: Die Anzahl Anfragen, die ein einzelner Gegenpart pro UTC-Tag an die eigene Instanz richten darf. Der Wert gilt für jeden Gegenpart einzeln und nicht für alle zusammen; ein leeres Feld bedeutet unbegrenzt. Siehe [Abfragelimits](../requestlimits/).
...
### Begrenzung der Anfragen eines Gegenparts
GTNet begrenzt an zwei Stellen, was ein Gegenpart verlangen darf: Das **Tägliche Abfragelimit** beschränkt die Anzahl seiner Anfragen pro UTC-Tag, das **Abfragelimit** je Austauschart die Anzahl Instrumente in einer einzelnen Anfrage. Nur das zweite gilt als Verletzung und führt bei wiederholter Missachtung zur Abweisung des Gegenparts, bis ein Administrator eingreift. Beide Begrenzungen, ihre Wirkung und was der Administrator davon zu sehen bekommt, sind unter [Abfragelimits](../requestlimits/) beschrieben.
...
Wurde eine Instanz ohne diesen Export umgezogen oder ihre Datenbank neu aufgebaut, sind die Zugangsdaten der Gegenparts verloren und die betroffenen Verbindungen lassen sich nicht mehr nutzen. Sie können nachträglich wiederhergestellt werden; das Vorgehen ist unter [Verbindung wiederherstellen](troubleshooting/) beschrieben.

### Aufgeklappte Zeile einer Instanz
Wird eine Zeile der Server-Übersicht aufgeklappt, erscheinen vier zugeklappte Bereiche, die je nach Bedarf einzeln geöffnet werden: **«Entity-Konfiguration»** mit den Austauscheinstellungen je Informationsobjekt, **«GT Message»** mit der Nachrichtenhistorie, **«Wartungsfenster»** mit den von dieser Instanz angekündigten Wartungszeiten und – nur für Administratoren – **«Zustellversuche»** mit dem Ergebnis ausgehender Hintergrundnachrichten. Jeder Bereich nennt in seinem Titel die Anzahl der enthaltenen Einträge, sodass bereits ohne Öffnen ersichtlich ist, wo etwas vorhanden ist.

Die Nachrichten werden in einer Hierarchie dargestellt, wobei zusammengehörige Nachrichten gruppiert werden; jede Nachricht zeigt Richtung (gesendet/empfangen), Nachrichtentyp und Status. Die Zustellversuche ergänzen dazu je Ziel die Spalten «Nachrichtenzeitpunkt», «Nachrichtencode», «Zieldomäne», «Versuchsstatus», «Versuche», «Letzter Versuch», «Zugestellt am» und «Letzter Fehler». Wie diese Angaben bei der Fehlersuche zu lesen sind, beschreibt [GTNet überwachen](../monitoring/#zustellversuche-von-nachrichten).
...
stateDiagram-v2
    ...
    Verbunden --> KeineVerbindung: Neuen Handshake erlauben
    DatenaustauschAktiv --> KeineVerbindung: Neuen Handshake erlauben
    ...
    note right of Verbunden : Token-Erneuerung möglich
    note right of DatenaustauschAktiv : Token-Erneuerung möglich
{{< /mermaid >}}

Nach der ersten Kontaktaufnahme und einer erfolgreichen Antwort werden Authentifizierungstoken ausgetauscht und die Verbindung ist hergestellt. Ab diesem Zeitpunkt kann ein Datenaustausch angefragt werden. Wird die Anfrage akzeptiert, erfolgt der automatische Austausch von Preisdaten. Der Datenaustausch kann jederzeit durch eine «Datenaustausch widerrufen»-Nachricht beendet werden. Token können bei bestehender Verbindung jederzeit erneuert werden. Aus jedem verbundenen Zustand führt zudem **«Neuen Handshake erlauben»** zurück auf «Keine Verbindung»; das ist der Weg, einen Gegenpart wieder zuzulassen, der seine Zugangsdaten verloren hat.
...
Für **Broadcast-Nachrichten** (an alle verbundenen Peers gleichzeitig) stehen die folgenden Optionen zur Verfügung: «Server ist jetzt offline» ist immer verfügbar. «Server befindet sich während des Zeitraums im Wartungsmodus» ist ebenfalls immer verfügbar, denn es dürfen mehrere Wartungsfenster gleichzeitig angekündigt sein; abgewiesen wird lediglich ein Fenster, das sich mit einem bereits angekündigten überschneidet. «Der Serverbetrieb wird ab diesem Datum eingestellt» erscheint dagegen nur, wenn keine offene Betriebseinstellung vorhanden ist. Beide Ankündigungen sind unter [Wartung und Betriebseinstellung](availability/) beschrieben.
...
Die beiden Ankündigungen zur geplanten Nichtverfügbarkeit können nach dem Versenden wieder aufgehoben werden. Dazu wählt man die gesendete Ankündigung in der Nachrichtenliste des eigenen Eintrags aus und ruft im Kontextmenü **«Widerrufen»** auf; gesendet wird dann «Wartung abgesagt» beziehungsweise «Betriebseinstellung abgesagt». Der Menüpunkt erscheint nur, solange der angekündigte Zeitpunkt noch in der Zukunft liegt. Damit werden die verbundenen Peers darüber informiert, dass der angekündigte Zustand nicht mehr gilt. Siehe [Wartung und Betriebseinstellung](availability/).
