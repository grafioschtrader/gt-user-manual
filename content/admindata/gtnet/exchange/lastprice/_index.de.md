...
##### Auswirkung des Wiederholungszählers
Nach dem GTNet-Austausch und vor dem Konnektor-Schritt aktualisiert GT den «Wiederholungszähler Innertag» der GTNet-fähigen Instrumente nach folgenden Regeln:

- Bei einem erfolgreichen GTNet-Empfang wird der Zähler auf höchstens «gt.intra.retry» begrenzt. Liegt er bereits darunter, bleibt er unverändert. Liegt er im Fallback-Band, wird er auf exakt «gt.intra.retry» gesetzt.
- Bei ausbleibender GTNet-Antwort und einem Zähler im Fallback-Band wird er um 1 erhöht, höchstens jedoch bis «gt.intra.retry + gt.gtnet.quote.retry».
- Liegt der Zähler unter «gt.intra.retry», bleibt er unverändert. Der Konnektor in Schritt 5 entscheidet dann selbst, ob er auf 0 zurücksetzt (Erfolg) oder erhöht (Misserfolg).

Damit erhalten Instrumente weiterhin aktuelle Preise, sobald GTNet liefert, während der Konnektor-Ausfall für die Überwachung sichtbar bleibt. Eine ausführliche Erläuterung der Bänder finden Sie unter [Wiederholungszähler und GTNet-Fallback](../../quoteretry/).
