...
##### Effect on the retry counter
After the GTNet exchange and before the connector step, GT updates the "Intraday retries counter" of the GTNet-enabled instruments according to the following rules:

- On a successful GTNet response, the counter is capped at `gt.intra.retry`. If it is already below that value, it remains unchanged. If it is in the fallback band, it is set to exactly `gt.intra.retry`.
- If GTNet does not respond and the counter is in the fallback band, it is incremented by 1, up to a maximum of `gt.intra.retry + gt.gtnet.quote.retry`.
- If the counter is below `gt.intra.retry`, it is left alone. The connector in step 5 will then decide whether to reset it to 0 (on success) or increment it (on failure).

This way instruments continue to receive current prices as soon as GTNet delivers, while the connector failure remains visible to monitoring. A detailed explanation of the bands can be found under [Retry Counter and GTNet Fallback](../../quoteretry/).
