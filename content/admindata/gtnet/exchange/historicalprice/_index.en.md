...
{{% /notice %}}
The exchange of historical price data allows sharing and receiving past closing prices with other GTNet instances. This exchange is integrated into the regular historical quote loading process and works automatically for instruments that have the **"Receive historical price data"** option enabled.

### Enabling GTNet for Historical Prices
To receive historical prices via GTNet for a specific security or currency pair, the **"Receive historical price data"** option must be enabled on that instrument. This can be configured in the instrument's settings.
...
flowchart TD
    ...
    B -->|Yes| D[Filter by 'Receive historical price data']
...
### GTNet as fallback when the connector has failed
In addition to the workflow shown above, GTNet has a fallback role that runs independently of the regular update. Instruments whose "Historical retries counter" has reached the `gt.history.retry` limit are picked up through a separate query, provided the "Receive historical price data" option is enabled. For these instruments GTNet keeps trying to deliver missing prices even though the connector has already given up.

When storing the results, a special rule applies: a successful GTNet response does not reset the retry counter to 0; at most it caps the counter at `gt.history.retry`. The connector failure therefore remains visible to monitoring while the instruments continue to receive current price data. If GTNet itself fails, the counter is incremented by 1. Once the sum `gt.history.retry + gt.gtnet.quote.retry` is reached, no further automatic attempts take place. A complete description of the behaviour and configuration values can be found under [Retry Counter and GTNet Fallback](../../quoteretry/).

### Detailed Workflow

#### Step 1: Filter by "Receive historical price data"
Only instruments with the "Receive historical price data" option enabled are candidates for GTNet exchange. Other instruments skip directly to connector-based loading.
...
- Instruments without the "Receive historical price data" option enabled
...
#### How It Works
When an Open server cannot provide data for a requested instrument but wants to receive data for this instrument (because the **"Receive historical price data"** option is enabled for the instrument), it sends back a "want to receive" marker:
...
| Enable Option | "Receive historical price data" | "Receive intraday price" |
