...
{{% /notice %}}
Der Austausch von historischen Kursdaten ermöglicht es, Schlusskurse der Vergangenheit mit anderen GTNet-Instanzen zu teilen und zu empfangen. Dieser Austausch ist in den regulären Prozess zum Laden historischer Kurse integriert und funktioniert automatisch für Instrumente, bei denen die Option **«Erhalte historische Preisdaten»** aktiviert ist.

### GTNet für historische Kurse aktivieren
Um historische Kurse über GTNet für ein bestimmtes Wertpapier oder Währungspaar zu empfangen, muss die Option **«Erhalte historische Preisdaten»** für dieses Instrument aktiviert sein. Dies kann in den Instrumenteinstellungen konfiguriert werden.
...
### GTNet als Fallback bei ausgefallenem Konnektor
Zusätzlich zum oben dargestellten Ablauf besitzt GTNet eine Fallback-Rolle, die unabhängig von der regulären Aktualisierung läuft. Instrumente, deren «Wiederholungszähler historisch» das Limit «gt.history.retry» erreicht hat, werden über eine separate Abfrage erfasst, sofern die Option «Erhalte historische Preisdaten» aktiviert ist. Für diese Instrumente versucht GTNet weiterhin, fehlende Kurse zu liefern, obwohl der Konnektor bereits ausgesetzt hat.

Bei der Speicherung gilt eine besondere Regel: Ein erfolgreicher GTNet-Empfang setzt den Wiederholungszähler nicht auf 0 zurück, sondern höchstens auf den Wert «gt.history.retry». Damit bleibt der Konnektor-Ausfall in der Überwachung sichtbar, während die Instrumente weiterhin mit aktuellen Kursdaten versorgt werden. Bleibt GTNet selbst erfolglos, wird der Zähler um 1 erhöht. Sobald die Summe «gt.history.retry + gt.gtnet.quote.retry» erreicht ist, finden keine automatischen Versuche mehr statt. Eine vollständige Beschreibung des Verhaltens und der Konfigurationswerte finden Sie unter [Wiederholungszähler und GTNet-Fallback](../../quoteretry/).

### Detaillierter Ablauf

#### Schritt 1: Filter für auszutauschende Preise
Nur Instrumente mit aktivierter Option **«Erhalte historische Preisdaten»** sind Kandidaten für den GTNet-Austausch. Andere Instrumente springen direkt zum Connector-basierten Laden.
...
- Instrumente ohne aktivierte Option «Erhalte historische Preisdaten»
...
#### Funktionsweise
Wenn ein Offen-Server keine Daten für ein angefragtes Instrument bereitstellen kann, aber selbst Daten für dieses Instrument empfangen möchte (weil die Option **«Erhalte historische Preisdaten»** für das Instrument aktiviert ist), sendet er eine «Empfangswunsch»-Markierung zurück:
...
| Aktivierungsoption | «Erhalte historische Preisdaten» | «Erhalte Intraday Preis» |
