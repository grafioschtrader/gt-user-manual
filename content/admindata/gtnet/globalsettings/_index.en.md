...
The GTNet functionality is controlled via specific global parameters. These parameters can be found in the [Global Settings](../../globalsettings/) view. They carry two different prefixes: "g.gnet" for the parameters of the GTNet infrastructure itself, and "gt.gtnet" for those that concern the exchange of price data.
...
The valid range for all three values is 1-10 days. The default value is "LP=1,HP=5,SL=5", meaning last price messages are deleted after 1 day, and historical price and instrument lookup messages after 5 days each. This cleanup is performed by [background task 22](../../../admindata/taskdatachangemonitor/taskdescription/#22---aggregate-gtnet-exchange-logs-and-delete-old-messages).
...
How these messages are entered and how the peers act on them is described under [Maintenance and Discontinuation](../setup/availability/).
...
1. If `g.gnet.use` is enabled (value > 0), GTNet functionality is activated
