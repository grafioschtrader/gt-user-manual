...
### Währungsuntereinheiten (GBp, GBX, ZAc)
Einige Handelsplattformen weisen Kurse von Wertpapieren der London Stock Exchange in Pence (GBp bzw. GBX) oder der Johannesburg Stock Exchange in Cents (ZAc) aus. Beim Import werden solche Werte automatisch in die Hauptwährung GBP bzw. ZAR umgerechnet, d.h. der Kurs wird durch 100 geteilt und die Währung entsprechend ersetzt. Die Importposition und die daraus erstellte Transaktion enthalten somit immer die Hauptwährung. Für die manuelle Erfassung siehe [Wertpapiertransaktion](../../../transaction/security/).

### Währungspaare beim Import
Enthält eine Importposition einen Wechselkurs, weil Wertpapier- und Kontowährung auseinanderfallen oder ein Kontoübertrag zwischen zwei Währungen vorliegt, benötigt GT das entsprechende **Währungspaar**. Ist es noch nicht vorhanden, wird es beim Verarbeiten der Position automatisch erstellt und mit Kursdaten versorgt, siehe [Währungspaar und Kryptowährungen](../../../watchlistinstrument/instrument/currencypair/).
...
### Transaktionslimit beim Import
Beim Import wird das [Transaktionslimit pro Klient](../../../transaction/) berücksichtigt. Es werden so viele Positionen importiert, wie unter die Grenze passen. Bereits erstellte Transaktionen bleiben erhalten und werden nicht rückgängig gemacht. Positionen, die das Limit überschreiten würden, werden nicht importiert und verbleiben in der Importliste, sodass sie nach dem Löschen bestehender Transaktionen später nachgeholt werden können. Ein Hinweis informiert Sie, wie viele Positionen aus diesem Grund nicht importiert wurden.
