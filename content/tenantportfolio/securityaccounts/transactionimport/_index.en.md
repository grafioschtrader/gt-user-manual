...
## Currency minor units (GBp, GBX, ZAc)
Some trading platforms show prices of securities on the London Stock Exchange in pence (GBp or GBX) or on the Johannesburg Stock Exchange in cents (ZAc). During the import, such values are automatically converted into the main currency GBP or ZAR, i.e. the price is divided by 100 and the currency is replaced accordingly. The import position and the transaction created from it therefore always contain the main currency. For manual entry, see [Securities transaction](../../../transaction/security/).

## Currency pairs during the import
If an import position contains an exchange rate, because the security and account currency differ or because it is an account transfer between two currencies, GT needs the corresponding **currency pair**. If it does not exist yet, it is created automatically while the position is processed and supplied with price data, see [Currency pair](../../../watchlistinstrument/instrument/currencypair/).
...
### Transaction limit during import
The [per-client transaction limit](../../../transaction/) is respected during import. As many positions as fit under the limit are imported. Transactions that have already been created are kept and are not rolled back. Positions that would exceed the limit are not imported and remain in the import list, so they can be imported later after existing transactions have been deleted. A notice informs you how many positions were not imported for this reason.
