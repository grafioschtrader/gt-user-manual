...
The GitHub repository [gt-pdf-transform](https://github.com/grafioschtrader/gt-pdf-transform) contains information on the installation and creation of **GT-PDF-Transform**.
...
In addition to the **menu bar**, the user interface is divided into three areas: **Files**, **Replacement** and **PDF**. The menu bar also contains a **Help** entry that opens this online manual in your default browser.
...
The context menu additionally offers the **"Anonymize number"** function. It is intended for the common case where a fixed label is followed by a **number that changes from document to document** (e.g. `Kontonummer 43332202` or `Unsere Referenz: 174634516`). Executed on a selection such as `Kontonummer 43332202`, it creates a **regular-expression** entry with the source text `Kontonummer \d+` and a target text that masks the digits (`Kontonummer 88888888`). Because the source matches the digits generically via `\d+`, the same rule anonymizes the corresponding number in **every** document and at **every** position where it occurs. The generated target text remains editable.

#### Edit replacement
The table rows in the **Replacement** area can be edited. A single click on the corresponding cell changes the value of the cell in the **selection box** or activates the **edit mode** for a **text input field**. The **Enter key** is used for the line break, so **Shift** + **Enter** **accepts** the **cell value** and ends editing. The use of [regular expressions](https://www.tutego.de/blog/javainsel/2018/09/einfuehrung-in-regulaere-ausdruecke-mit-der-java-api/) is possible.

The **Regular expression** checkbox marks a rule as a regular expression: when checked, the **source text** is interpreted as a search pattern (e.g. `\d+`) and the **target text** may use back-references such as `$1`. The "Anonymize number" function sets this checkbox automatically.

{{% notice info %}} 
For those familiar with the Java programming language, the replaceAll method is used for the replacement. As a result, **every** occurrence of a match is replaced in each document, not just the first. 
{{% /notice %}}
