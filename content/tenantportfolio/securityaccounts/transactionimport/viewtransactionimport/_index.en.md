...
date: 2026-06-29T22:54:47+01:00
draft: false
weight: 15
archetype: "default"
---
...
- **Total amount check**: The **total amount** is calculated from the details of the imported transaction; this must match the corresponding value of the **"ta" field**. A rounding tolerance configured in the [import template](../../../basedata/imptranstemplate/createimptranstemplate/) (**calcRounding**) is accepted here.
...
- **Accept difference total**: The small deviation between the calculated total and the total shown in the document is accepted and the corresponding import item becomes **transaction-enabled**. When the transaction is created, the total shown in the document is credited to the bank account so that the balance matches the statement, and the deviation is recorded as a **rounding difference** on the transaction. This function should only be used if the deviation is very small. If the **calcRounding** configuration is set in the [import template](../../../basedata/imptranstemplate/createimptranstemplate/), the system already accepts such deviations within the defined tolerance automatically, so this step is no longer necessary.
...
- **Create transaction**: The transformation of an import position into a transaction is only possible if the import position **does not** yet have a **transaction** and no "**maybe transaction**". If the date of an import position is after the **Active until or maturity date** of the assigned account or securities account, the creation for that position is rejected with an error message and skipped; the remaining positions are still processed. See [Deactivating an account]({{% ref "/tenantportfolio/cashaccount#deactivating-an-account" %}}).
