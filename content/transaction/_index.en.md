...
The total number of transactions per client is limited. The administrator sets the maximum via the global setting **Number of possible transactions per client** (default 5000, see [Global settings](../admindata/globalsettings/)). Once this limit is reached, no further transactions can be added until existing ones are deleted or the administrator raises the maximum. Deleting transactions frees up space again, because the current count is always evaluated.
...
- **Import**: as many positions as fit under the limit are imported. The remaining positions are not imported but stay in the import list, and a corresponding notice is shown.
