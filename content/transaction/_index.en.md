---
title: "Transaction"
date: 2026-08-15T02:54:47+01:00
draft: false
weight: 20
archetype: "default"
---
A distinction is made between **transactions** that relate only to **bank accounts** and those involving an **instrument**. In addition, some transactions can be **imported** via **PDF** or **CSV** but can certainly be **entered manually**. For existing **security transactions**, GT can also generate its own PDF receipts, see [Transaction receipts](./receipt/).
...
If two currencies meet in a transaction, that is in an account transfer between two currencies or when the instrument and account currency differ, GT needs a **currency pair** for the exchange rate. If it does not exist yet, it is created automatically during entry, see [Currency pair](../watchlistinstrument/instrument/currencypair/).
...
- **Closed until**: Transactions whose transaction time lies on or before the effective **Closed until** date can neither be created nor changed nor deleted. The date is taken from the [portfolio](../tenantportfolio/portfolio/) or, as a fallback, from the [client](../tenantportfolio/client/#properties). Exempt are the functions **Toggle taxable status** and **Set ex-date from tax data** of the [Dividends and interest](../reportportfolio/dividends/) report, as these only change tax-related attributes.

## Maximum number of transactions per client
The total number of transactions per client is limited. The administrator sets the maximum through the limit **Transaction** of the limit type **Total number**, which is counted by **Client** (default 5000, see [Limit information class]({{% relref "/admindata/entitylimit" %}})). Once this limit is reached, no further transactions can be added until existing ones are deleted or the administrator raises the maximum. Deleting transactions frees up space again, because the current count is always evaluated.

{{% notice style="info" title="Behaviour when the limit is reached" %}}
GT behaves differently depending on where a transaction originates:
- **Manually entered single transaction**: entry is rejected with an error message.
- **Account transfer and security action** (ISIN change, security transfer): these create several related transactions at once. They are rejected if the limit is already reached. If the count is still below the limit, the operation is carried out in full, even if this slightly exceeds the limit.
- **Import**: before the first write operation it is checked whether the whole import fits under the limit. If it does not, the import is rejected in full and not a single transaction is taken over; the message names the limit, the existing stock and the number of transactions to be added. An import that only updates existing transactions always succeeds, because it adds nothing.
- **Standing order**: automatic executions are also stopped when the limit is reached. The affected execution is recorded as a failure and is resumed automatically on the next run once space is available again.
{{% /notice %}}
...
A compromise was found between the Iranian Rial and Bitcoin currencies for the numerical format of amounts and prices. The pre-decimal places are based on the Iranian rial and the decimal places on Bitcoin.
...
Most currencies have a subdivision of 100, such as the CHF, EUR and USD. The Kuwait Dinar is a thousand-dollar currency, so an accuracy of 3 decimal places is expected. For cryptocurrencies such as Bitcoin, 8 decimal places are expected. Finally, there are also currencies that do not use decimal places, such as the Japanese yen. These different accuracies are covered by the global setting «gt.currency.precision», which the administrator can adjust in the [global settings](../admindata/globalsettings/). Currencies without an entry are handled with 2 decimal places. This accuracy is taken into account when validating the total amount of a transaction and in the automatic correction described below.

{{% notice style="info" title="Automatic correction of exchange rate or Price/Div/etc." %}}
Rounded or truncated decimal places may be specified in the transaction documents. Based on the decimal places of a currency, the GT backend attempts to achieve the desired amount of the account entry. For example, from the account entry CHF 2887.84995323 an amount of CHF 2887.85 is sought by adjusting the exchange rate, if available, or otherwise by adjusting the Price/Div/etc. In reality, an amount of CHF 2887.84999998 may be reached in this example.
{{% /notice %}}
