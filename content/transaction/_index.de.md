...
archetype: "default"
---
Es wird unterschieden zwischen **Transaktionen** die sich nur auf **Bankkonten** beziehen oder solche bei denen ein **Instrument** beteiligt ist. Zudem können Transaktionen teilweise mittels **PDF** oder **CSV importiert** aber sicherlich **manuell erfasst** werden. Für bestehende **Wertpapiertransaktionen** kann GT zudem eigene PDF-Belege erzeugen, siehe [Transaktionsbelege](./receipt/).
...
Fallen bei einer Transaktion zwei Währungen zusammen, also bei einem Kontoübertrag zwischen zwei Währungen oder wenn Instrument- und Kontowährung auseinanderfallen, benötigt GT ein **Währungspaar** für den Wechselkurs. Ist dieses noch nicht vorhanden, wird es beim Erfassen automatisch erstellt, siehe [Währungspaar und Kryptowährungen](../watchlistinstrument/instrument/currencypair/).
...
- **Geschlossen bis**: Transaktionen, deren Transaktionszeit an oder vor dem massgebenden Datum **Geschlossen bis** liegt, können weder erstellt noch geändert oder gelöscht werden. Das Datum stammt aus dem [Portfolio](../tenantportfolio/portfolio/) oder ersatzweise vom [Klienten](../tenantportfolio/client/#eigenschaften). Ausgenommen sind die Funktionen **Steuerstatus umschalten** und **Ex-Tag aus Steuerdaten setzen** der Auswertung [Dividende und Zins](../reportportfolio/dividends/), da diese nur steuerliche Angaben ändern.

## Maximale Anzahl Transaktionen pro Klient
Die Gesamtzahl der Transaktionen pro Klient ist begrenzt. Den Höchstwert legt der Administrator über die Limite **Transaktion** der Limitenart **Gesamtanzahl** fest, welche nach **Mandant** gezählt wird (Standardwert 5000, siehe [Limite Informationsklasse]({{% relref "/admindata/entitylimit" %}})). Ist diese Grenze erreicht, können keine weiteren Transaktionen mehr hinzugefügt werden, bis bestehende Transaktionen gelöscht werden oder der Administrator den Höchstwert erhöht. Durch das Löschen von Transaktionen wird wieder Platz frei, da immer der aktuelle Bestand gezählt wird.

{{% notice style="info" title="Verhalten beim Erreichen der Grenze" %}}
Je nach Herkunft einer Transaktion verhält sich GT beim Erreichen der Grenze unterschiedlich:
- **Manuell erfasste Einzeltransaktion**: Die Erfassung wird mit einer Fehlermeldung abgelehnt.
- **Kontoübertrag und Wertpapieraktion** (ISIN-Änderung, Wertpapiertransfer): Diese erzeugen mehrere zusammengehörige Transaktionen auf einmal. Sie werden abgelehnt, wenn die Grenze bereits erreicht ist. Liegt der Bestand noch darunter, wird die Aktion vollständig ausgeführt, auch wenn die Grenze dadurch geringfügig überschritten wird.
- **Import**: Bereits vor dem ersten Schreibvorgang wird geprüft, ob der ganze Import unter die Grenze passt. Ist das nicht der Fall, so wird der Import vollständig abgelehnt und es wird keine einzige Transaktion übernommen; die Meldung nennt die Grenze, den bestehenden Bestand und die Anzahl der hinzukommenden Transaktionen. Ein Import, der ausschliesslich bestehende Transaktionen aktualisiert, gelingt immer, da er nichts hinzufügt.
- **Dauerauftrag**: Auch automatische Ausführungen werden beim Erreichen der Grenze gestoppt. Die betroffene Ausführung wird als Fehler vermerkt und beim nächsten Lauf automatisch wieder aufgenommen, sobald wieder Platz vorhanden ist.
{{% /notice %}}
...
Die meisten Währungen haben eine Unterteilung von 100, wie beispielsweise der CHF, EUR, USD. Der Kuwait-Dinar ist eine Tausenderwährung, somit wird eine Genauigkeit von 3 Nachkommastellen erwartet. Bei Kryptowährungen wie dem Bitcoin werden 8 Nachkommastellen erwartet. Schliesslich gibt es auch Währungen die auf Nachkommastellen verzichten, dazu gehört der Japanische Yen. Diese unterschiedlichen Genauigkeiten werden über die globale Einstellung «gt.currency.precision» abgebildet, die der Administrator in den [globalen Einstellungen](../admindata/globalsettings/) anpassen kann. Währungen ohne Eintrag werden mit 2 Nachkommastellen geführt. Diese Genauigkeit wird bei der Prüfung des Totalbetrags einer Transaktion und bei der nachfolgend beschriebenen automatischen Korrektur berücksichtigt.
