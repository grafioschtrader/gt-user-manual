...
- Wenn das Depot [Handelsperioden]({{% ref "/tenantportfolio/securityaccounts" %}}#handelsperioden) definiert hat, müssen der Instrumententyp des Wertpapiers und das Transaktionsdatum mit mindestens einer erlaubten Handelsperiode übereinstimmen. Andernfalls wird die Transaktion abgelehnt.
- Beim Erfassen einer Transaktion werden nur **Depots** und **Konten** zur Auswahl angeboten, die am gewählten **Transaktionsdatum** aktiv sind. Eine Buchung nach dem **Aktiv bis oder Fälligkeitstag** eines Kontos oder Depots wird abgelehnt. Siehe [Stilllegung eines Kontos]({{% ref "/tenantportfolio/cashaccount#stilllegung-eines-kontos" %}}).
...
### Hilfsfunktionen für den Wechselkurs
Neben dem Eingabefeld **Wechselkurs** stehen bis zu drei Schaltflächen zur Verfügung, die das Arbeiten mit Wechselkursen erleichtern.
- **⇄**: Aktualisiert den Wechselkurs gemäss der eingegebenen Transaktionszeit. Damit wird ein historischer Wechselkurs vom System übernommen, was in den meisten Fällen als guter Ausgangswert dient.
- **1/x**: Berechnet den Kehrwert des aktuell angezeigten Wechselkurses. Dies ist hilfreich, wenn der Transaktionsbeleg den Wechselkurs im umgekehrten Währungspaar ausweist.
- **f(x)**: Öffnet einen Dialog zur Eingabe des Gesamtbetrages der Kontobuchung. Daraus berechnet das System den passenden Wechselkurs. Dies ist besonders nützlich beim Schliessen von Forex-Positionen, wenn die Handelsplattform keinen expliziten Wechselkurs mitteilt, sondern nur den gutgeschriebenen Gesamtbetrag ausweist.

{{% notice note %}}
Die Schaltflächen **⇄** und **1/x** stehen auch beim [Kontoübertrag](../account) mit Währungskonvertierung zur Verfügung. Die Schaltfläche **f(x)** ist ausschliesslich bei Wertpapiertransaktionen vorhanden.
{{% /notice %}}

## Notierung in Pence und Cents (GBp, GBX, ZAc)
An der London Stock Exchange werden viele Wertpapiere in Pence (GBp bzw. GBX) statt in Pfund (GBP) notiert; die Johannesburg Stock Exchange verwendet Cents (ZAc) statt Rand (ZAR). GT führt jedes Wertpapier ausschliesslich in der Hauptwährung, also GBP oder ZAR. Die historischen und Intraday-Kurse werden von den Datenanbietern automatisch in die Hauptwährung umgerechnet. Ob ein Handelsbeleg Pence oder Pfund ausweist, hängt von der Handelsplattform ab: Dasselbe Wertpapier kann auf einer Plattform in GBp und auf einer anderen in GBP dokumentiert sein.
{{% notice note %}}
Bei der manuellen Erfassung einer Transaktion müssen Werte, die auf dem Beleg in Pence oder Cents ausgewiesen sind, vom Benutzer selbst durch 100 geteilt werden. Beispielsweise wird ein Kurs von 1600 GBp als 16.00 GBP erfasst. Eine vergessene Umrechnung fällt in der Regel bei der Prüfung der **Kontobuchung** auf, da die berechnete Buchung um den Faktor 100 vom Beleg abweicht. Beim [Transaktionsimport](../../tenantportfolio/securityaccounts/transactionimport/) erfolgt diese Umrechnung automatisch.
{{% /notice %}}
...
- **Währungsgewinn Hauptwährung**: Der Beitrag dieser einzelnen Transaktion zum Währungsgewinn der Position. Ein Wert wird für jede Transaktion ausgewiesen, also auch für Käufe, Dividenden und Marchzinsen. Die einzelnen Beträge einer Position ergeben zusammen den Währungsgewinn, der in den Berichten für dieses Instrument erscheint. Bei einem Instrument, das bereits in der Hauptwährung notiert, bleibt die Spalte leer. Siehe [Währungsgewinn]({{% ref "/reportportfolio#währungsgewinn" %}}).
