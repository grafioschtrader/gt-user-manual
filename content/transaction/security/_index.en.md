...
- If the securities account has [trading periods]({{% ref "/tenantportfolio/securityaccounts" %}}#trading-periods) defined, the instrument type of the security and the transaction date must match at least one permitted trading period. Otherwise, the transaction is rejected.
- When entering a transaction, only **securities accounts** and **accounts** that are active on the selected **transaction date** are offered for selection. An entry dated after the **Active until or maturity date** of an account or securities account is rejected. See [Deactivating an account]({{% ref "/tenantportfolio/cashaccount#deactivating-an-account" %}}).
...
## Quotation in pence and cents (GBp, GBX, ZAc)
On the London Stock Exchange, many securities are quoted in pence (GBp or GBX) instead of pounds (GBP); the Johannesburg Stock Exchange uses cents (ZAc) instead of rand (ZAR). GT keeps every security exclusively in the main currency, i.e. GBP or ZAR. The historical and intraday prices are automatically converted into the main currency by the data providers. Whether a trade document shows pence or pounds depends on the trading platform: the same security may be documented in GBp on one platform and in GBP on another.
{{% notice note %}}
When entering a transaction manually, values shown in pence or cents on the document must be divided by 100 by the user. For example, a price of 1600 GBp is entered as 16.00 GBP. A forgotten conversion is usually detected by the check of the **account booking**, as the calculated booking deviates from the document by a factor of 100. With the [transaction import](../../tenantportfolio/securityaccounts/transactionimport/), this conversion is performed automatically.
{{% /notice %}}
...
- **Forex gain main currency**: The contribution of this single transaction to the Forex gain of the position. A value is shown for every transaction, so for purchases, dividends and accrued interest as well. The individual amounts of a position add up to the Forex gain that appears for this instrument in the reports. For an instrument already denominated in the main currency the column stays empty. See [Forex gain]({{% ref "/reportportfolio#forex-gain" %}}).
