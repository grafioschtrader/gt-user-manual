...
archetype: "default"
---
**Instrumente Forex** und **CFD** sind **Margin basierte Transaktionen**. Dabei gibt es eine Transaktion, welche die Position **öffnet**, dazu eine oder mehrere Transaktionen die diese geöffnete Position **schliessen**. Erst das Schliessen der Position erzeugt eine kontorelevante Buchung.

{{% notice warning %}}
Die **Margin** oder **Hinterlegungssumme** ein wichtiger Teil des Hebelhandels wird in GT nicht berücksichtigt. Dies ist eine **Sicherheitseinlage** und keine Gebühr auch keine Transaktionskosten. Sie dient dazu das der Trader nicht jenseits seiner finanziellen Möglichkeiten hohe Handelsgeschäfte tätigt. **GT eignet sich nicht als Überwachungs-Tool für das Eingehen von hohen gehebelten Risiken**.
{{% /notice %}}
...
{{% notice warning %}}
Die **offenen Forex Positionen** werden zur Zeit **nicht** mit den bestehenden **Cash-Positionen**, d.h. das Geld auf den Kontos verrechnet.
{{% /notice %}}

#### CFD
Wenn die Preise an den Finanzmärkte nur steigen würden, dann gäbe es in **GT** wahrscheinlich keine Unterstützung für dieses Finanzinstrument. Dem ist aber nicht so. Mit **CFD** können Sie beispielsweise Ihre Aktienpositionen kurzzeitig absichern oder grundsätzlich auf fallenden Märkte setzen.
...
### Position schliessen
Beim Schliessen der Position muss eine der öffnende Position des Instrumentes selektiert sein. Diese werden mit der **expandierende Tabellenzeile** des Instrumentes sichtbar. Auf der oberen hierarchischen Struktur in der **Hierarchietabelle** werden die **öffenten Positionen** dargestellt. Die **Finanzierungskosten** und die **schliessenden Positionen** werden als Konten dieser **öffenten Postionen** dargestellt.
