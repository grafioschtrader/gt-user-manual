...
When a security standing order is executed, the historical closing price of the security is required. Beforehand, the execution date is moved by the [non-trading-day adjustment](../#non-trading-day-adjustment) to a day on which the associated stock exchange is open. The price is then looked up on that trading day, with the [quote tolerance](../#quote-tolerance) determining how many days GT may deviate to either side. If no price is found within that window either, the execution is skipped for that date and an entry is created in the [failure log](../#execution-failures).

### Calculation flow
The following flow shows how the background process calculates the securities transaction:
{{< mermaid >}}
graph TD
    N[Move execution date to a trading day of the exchange] --> A[Search historical closing price within the quote tolerance]
    A --> B{Price found?}
    B -- No --> C[Log failure]
...
{{% notice note %}}
The same search window applies to the exchange rate as to the security price: if the rate is missing on the execution date, GT moves to a neighbouring day within the [quote tolerance](../#quote-tolerance). If no exchange rate is found there either, the execution is skipped and logged, because a posting without conversion would produce a wrong amount.
{{% /notice %}}

## Input fields
When creating or editing a security standing order, the following information must be provided:
- **Transaction type**: Buy or sell.
- **Account**: The account through which the transaction is settled. An account deactivated via the **Active until or maturity date** field is not offered for selection depending on the **valid from** date (see [Deactivating an account]({{% ref "/tenantportfolio/cashaccount#deactivating-an-account" %}})).
...
## Restrictions
- **Price dependency**: Execution requires that historical price data for the security is available on the shifted trading day or within the [quote tolerance](../#quote-tolerance). If the price is missing, the date is skipped and an [execution failure](../#execution-failures) is logged.
- **Exchange rate dependency**: If a currency pair is configured, a historical exchange rate must also be available within the same window.
- **Trading calendar**: The shift to a trading day relies on the trading calendar stored in GT. If that calendar is incomplete for the exchange concerned, the execution may land on a day without a price.
