...
Bei der Ausführung eines Wertpapier-Dauerauftrags wird der historische Schlusskurs des Wertpapiers benötigt. Vorgängig wird das Ausführungsdatum über die [Anpassung an Handelstage](../#anpassung-an-handelstage) auf einen Tag verschoben, an dem die zugehörige Börse geöffnet ist. An diesem Handelstag wird der Kurs gesucht, wobei die [Kurstoleranz](../#kurstoleranz) bestimmt, wie viele Tage GT dabei nach beiden Seiten ausweichen darf. Findet sich auch innerhalb dieses Fensters kein Kurs, wird die Ausführung für diesen Termin übersprungen und ein Eintrag im [Fehlerprotokoll](../#ausführungsfehler) erstellt.

### Berechnungsablauf
Der folgende Ablauf zeigt, wie der Hintergrundprozess die Wertpapiertransaktion berechnet:
{{< mermaid >}}
graph TD
    N[Ausführungsdatum auf Handelstag der Börse verschieben] --> A[Historischen Schlusskurs innerhalb der Kurstoleranz suchen]
    A --> B{Kurs gefunden?}
...
{{% notice note %}}
Für den Wechselkurs gilt dasselbe Suchfenster wie für den Wertpapierkurs: Fehlt der Kurs am Ausführungsdatum, weicht GT im Rahmen der [Kurstoleranz](../#kurstoleranz) auf einen benachbarten Tag aus. Wird auch dort kein Wechselkurs gefunden, wird die Ausführung übersprungen und protokolliert, denn eine Buchung ohne Umrechnung würde einen falschen Betrag ergeben.
{{% /notice %}}
...
- **Konto**: Das Konto, über das die Transaktion abgerechnet wird. Ein über das Feld **Aktiv bis oder Fälligkeitstag** stillgelegtes Konto wird abhängig vom **Gültig-von**-Datum nicht zur Auswahl angeboten (siehe [Stilllegung eines Kontos]({{% ref "/tenantportfolio/cashaccount#stilllegung-eines-kontos" %}})).
...
## Einschränkungen
- **Kursabhängigkeit**: Die Ausführung setzt voraus, dass für das Wertpapier historische Kursdaten am verschobenen Handelstag oder innerhalb der [Kurstoleranz](../#kurstoleranz) vorhanden sind. Fehlt der Kurs, wird der Termin übersprungen und ein [Ausführungsfehler](../#ausführungsfehler) protokolliert.
- **Wechselkursabhängigkeit**: Ist ein Währungspaar konfiguriert, muss innerhalb desselben Fensters auch ein historischer Wechselkurs vorliegen.
- **Börsenkalender**: Die Verschiebung auf einen Handelstag stützt sich auf den in GT hinterlegten Börsenkalender. Ist dieser für die betreffende Börse unvollständig, kann die Ausführung auf einem Tag ohne Kurs landen.
